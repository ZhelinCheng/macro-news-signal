...
**网页：** 优先判断是否有 agent-browser 技能；若有，使用其进行动态访问；若无，使用默认方式抓取，例如web_fetch，网页抓取前请阅读对应网站的robots.txt，并遵守其爬虫协议及使用合适的速率。
**API：** 参见 `references/news_apis.md`

`references/news_apis.md`中的接口、RSS订阅源请使用curl方式请求（严格遵守请求规范），若环境没有curl时才回退到web_fetch：

```bash
curl 'references/news_apis.md中的接口或RSS订阅源' \
...
- 指标相关性： 结合 data_schema.md 中的宏观指标，计算当前新闻对特定资产（如黄金与 10Y TIPS 收益率背离）的映射强度。
